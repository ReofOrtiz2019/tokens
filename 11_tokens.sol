...
contract TTCOINtoken is IERC20 {
    ...
    address public admin;//NL
    uint public tokenPrice =100; //NL (Nueva linea)
    ...
    constructor() {
        admin = msg.sender;//NL
        _balances[address(this)] = _totalSupply / 2; //asignamos al contrato la otra mitad
        _balances[msg.sender] = _totalSupply / 2;//Asignamos al sender la mitad de los tokens emitidos
        emit Transfer(address(0), address(this), _balances[address(this)]); //NL
        emit Transfer(address(0), msg.sender, _balances[msg.sender]);//ML (Linea modificada)
    }
    ...
    /*Lineas nuevas*/
    //Funcion para establecer nuevo precio de os tokens
    function setTokenPrice(uint newPrecio) public {
        require(msg.sender == admin, "Solo el admin puede cambiar el precio");
        tokenPrice = newPrecio;

    }
    //Funcion para comprar tokens con ether
    function buyEnergytokens() public payable  {
        require(msg.value>0,"Debes enviar Ether para comprar tokens");
        
        uint256 tokensToBuy = msg.value * tokenPrice;
        require(_balances[address(this)]>=tokensToBuy,"No hay suficientes tokens disponibles");
        _balances[address(this)] -= tokensToBuy;
        _balances[msg.sender] += tokensToBuy;
        emit Transfer(address(this), msg.sender, tokensToBuy);
    }
    //funcion para que el administrador pueda retirar los fondos del contrato
}
